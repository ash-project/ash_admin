defmodule AshAdmin.Components.Resource.DataTable do
    ...
    if assigns[:initialized] do
      ...
      socket =
        if params["page"] && socket.assigns.action.pagination do
          ...
          page_params =
            page_from_params(params["page"], default_limit, !!count?)
      ...
      socket =
        if run_now? do
          if socket.assigns[:tables] != [] && !socket.assigns[:table] do
            ...
            case AshPhoenix.Form.submit(socket.assigns.query, api_opts: action_opts) do
  ...
  def handle_event("next_page", _, socket) do
    params =
      %{"page" => page_link_params(socket.assigns.data, "next")}
