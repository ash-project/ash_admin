...
defmodule AshAdmin.Components.Resource.CinderTable do
  ...
  attr :theme, :any, default: AshAdmin.Themes.AshAdminTheme
    ...
    <div class="ash-admin-cinder-table">
      ...
      >
        <:col
          ...
          sort={sortable?(@resource, attribute)}
          filter={filterable_type(@resource, attribute)}
          search
        >
          {render_cell(
            @domain,
  ...
  Checks DSL-configured list first, falls back to smart defaults based on attribute type.
  """
  def sortable?(resource, attribute) do
    case AshAdmin.Resource.table_sortable_columns(resource) do
      nil -> default_sortable?(attribute)
      columns -> attribute.name in columns
    end
  end

  defp default_sortable?(%module{}) when module in [HasOne, BelongsTo], do: false
  defp default_sortable?(%Ash.Resource.Calculation{}), do: false
  defp default_sortable?(%Ash.Resource.Aggregate{}), do: false
  defp default_sortable?(%Ash.Resource.Attribute{}), do: true
  defp default_sortable?(_), do: false
  ...
  """
  def filterable_type(resource, attribute) do
    case AshAdmin.Resource.table_filterable_columns(resource) do
      nil -> default_filterable_type(attribute)
      columns -> if attribute.name in columns, do: default_filterable_type(attribute), else: false
    end
  end

  defp default_filterable_type(%module{}) when module in [HasOne, BelongsTo], do: false
  defp default_filterable_type(%Ash.Resource.Calculation{}), do: false
  defp default_filterable_type(%Ash.Resource.Aggregate{}), do: false

  defp default_filterable_type(%Ash.Resource.Attribute{type: type}) do
    cond do
  ...
  defp default_filterable_type(_), do: false
