defmodule AshAdmin.Components.TopNav.ActorSelect do
  ...
  def render(assigns) do
    ...
    <div
    id="actor-hook"
    class="relative mr-5 text-white"
    phx-hook="Actor">
      <div :if={{ @actor }}>
        <span :if={{ @actor }}>
          <button :on-click={{ @toggle_authorizing }} type="button">
            <svg :if={{ @authorizing }} width="1em" height="1em" viewBox="0 0 16 16" fill="white" xmlns="http://www.w3.org/2000/svg">
          ...
          <button :on-click={{ @toggle_actor_paused }} type="button">
              <svg :if={{ @actor_paused }} width="1em" height="1em" viewBox="0 0 16 16" fill="white" xmlns="http://www.w3.org/2000/svg">
