defmodule AshAdmin.Components.TopNav.TenantForm do
  ...
  def render(assigns) do
    ~H"""
    <div id="tenant-hook" phx-hook="Tenant" style="margin-right: 20px;">
      <form :if={{@editing_tenant}} :on-submit={{@set_tenant}}>
        <input type="text" name="tenant" value={{ @tenant }}>
        <button class="btn" :on-click="stop_editing_tenant">
      ...
      <button :if={{ @tenant }} class="btn" :on-click={{@clear_tenant}}>
